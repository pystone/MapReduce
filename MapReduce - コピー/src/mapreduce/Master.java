...
public class Master {
	...
	private Master() {
		...
		ListenThread l = new ListenThread(GlobalInfo.sharedInfo().MasterPort);
		l.start();
		
//		JobInitThread initJobs = new JobInitThread();
//		new Thread(initJobs).start();
	}
	...
	public void newTask(String inputFile, String mapperPath,
		...
		try {
			String host = GlobalInfo.sharedInfo().DataMasterHost;
			int port = GlobalInfo.sharedInfo().DataMasterPort;
			Registry registry = LocateRegistry.getRegistry(host, port);
            KPFSMasterInterface stub = (KPFSMasterInterface) registry.lookup("KPFSMasterInterface");
			
			ArrayList<String> files = stub.splitFile(inputFile,
